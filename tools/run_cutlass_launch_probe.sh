...
write_env_file() {
  ...
  local strategy="$2"
  {
    ...
    printf 'CMAKE_CUDA_ARCHITECTURES=120\n'
...
extract_case_evidence() {
  local dir="$1"
  grep -hE '\[UXSCHED-(HB|XQUEUE)\].*(split_count|backend_selected=HB_SPLIT|transform_succeeded|transformed_module_loaded|parent_launch_submitted|child_launch_submitted|child_launch_completed|split_group_completed|parent_launch_completed|HIGH_PRIORITY_PASSTHROUGH|backend_selected=NATIVE|NO_XQUEUE|KERNEL_NOT_VERIFIED|PTX_UNAVAILABLE|runtime_strategy)' \
...
write_summary() {
  ...
  if [[ "${hb_transform}" =~ ^[0-9]+$ && "${hb_parent}" =~ ^[0-9]+$ &&
    ...
    if (( hb_transform == 0 && hb_parent == 0 && hb_child == 0 && hb_transformed == 0 &&
          hb_fallback == 0 && hb_noxq == 0 )); then
